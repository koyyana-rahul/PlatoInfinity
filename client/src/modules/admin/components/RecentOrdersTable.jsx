  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-2.5 sm:gap-3">
        <h3 className="text-base sm:text-lg font-bold text-slate-900">Recent Orders</h3>

        <div className="grid grid-cols-2 sm:grid-cols-3 gap-1.5 sm:gap-2 w-full lg:w-auto">
          <div className="col-span-2 sm:col-span-1 text-[11px] sm:text-sm text-slate-600 bg-slate-100 px-2.5 sm:px-3 py-2 rounded-md text-center sm:text-left whitespace-nowrap">
          ...
          <button
            ...
            onClick={() => setCompactMode((prev) => !prev)}
              className="w-full text-[11px] sm:text-sm border border-slate-200 rounded-md px-2.5 sm:px-3 py-2 bg-white text-slate-700 hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-orange-200"
          >
      ...
      <div className="bg-white border border-slate-100 rounded-lg overflow-hidden">
        ...
        <div className="xl:hidden divide-y divide-slate-100 max-h-[420px] overflow-y-auto">
          {paginatedOrders.map((order) => {
            const sourceTs = order.createdAt || order.updatedAt;
            const dateTime = formatOrderDateTimeParts(sourceTs);

            return (
              <div key={order._id} className={compactMode ? "p-2.5 space-y-2" : "p-3.5 space-y-2.5"}>
